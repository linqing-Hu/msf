 ...
 * @LastEditTime: 2022-11-05 09:57:29
 */
...
ICPRegistration::ICPRegistration(const YAML::Node& node)
    : icp_ptr_(new pcl::IterativeClosestPoint<CloudData::PointType,
                                              CloudData::PointType>()) {
...
bool ICPRegistration::SetInputTarget(
...
bool ICPRegistration::ScanMatch(const CloudData::CloudTypePtr& input_source,
...
float ICPRegistration::GetFitnessScore() { return icp_ptr_->getFitnessScore(); }

bool ICPRegistration::IsDegeneracy() {
  // 这种判断退化的方式非常不准
  return (icp_ptr_->getFitnessScore() > 10);
}
