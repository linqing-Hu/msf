 ...
 * @LastEditTime: 2022-10-03 19:43:17
 */
#include "lidar_localization/mapping/back_end/lio_back_end_sim.h"

namespace lidar_localization {

LIOBackEndSim::LIOBackEndSim() { InitWithConfig(); }

bool LIOBackEndSim::InitWithConfig() {
  const std::string config_file_path =
      WORK_SPACE_PATH + "/config/mapping/lio_back_end_sim.yaml";
...
bool LIOBackEndSim::InitDataPath(const YAML::Node& config_node) {
...
bool LIOBackEndSim::InitParam(const YAML::Node& config_node) {
  key_frame_distance_ = config_node["key_frame_distance"].as<float>();

  return true;
}

bool LIOBackEndSim::InitGraphOptimizer(const YAML::Node& config_node) {
...
bool LIOBackEndSim::InitIMUPreIntegrator(const YAML::Node& config_node) {
  imu_pre_integrator_ptr_ = nullptr;

  if (graph_optimizer_config_.use_imu_pre_integration) {
    imu_pre_integrator_ptr_ =
        std::make_shared<IMUPreIntegrator>(config_node["imu_pre_integration"]);
  }

  return true;
}

bool LIOBackEndSim::InitOdoPreIntegrator(const YAML::Node& config_node) {
  odo_pre_integrator_ptr_ = nullptr;

  if (graph_optimizer_config_.use_odo_pre_integration) {
    odo_pre_integrator_ptr_ =
        std::make_shared<OdoPreIntegrator>(config_node["odo_pre_integration"]);
  }

  return true;
}

bool LIOBackEndSim::InsertLoopPose(const LoopPose& loop_pose) {
  ...
  LOG(INFO) << "Add loop closure: " << loop_pose.index0_ << ","
            << loop_pose.index1_ << std::endl;

  return true;
}

bool LIOBackEndSim::UpdateIMUPreIntegration(const IMUData& imu_data) {
  if (!graph_optimizer_config_.use_imu_pre_integration ||
...
bool LIOBackEndSim::UpdateOdoPreIntegration(const VelocityData& velocity_data) {
  if (!graph_optimizer_config_.use_odo_pre_integration ||
...
bool LIOBackEndSim::Update(const PoseData& laser_odom,
                           const PoseData& gnss_pose,
                           const IMUData& imu_data) {
  ResetParam();

  if (MaybeNewKeyFrame(laser_odom, gnss_pose, imu_data)) {
...
void LIOBackEndSim::ResetParam() {
  has_new_key_frame_ = false;
  has_new_optimized_ = false;
}

bool LIOBackEndSim::MaybeNewKeyFrame(const PoseData& laser_odom,
                                     const PoseData& gnss_pose,
                                     const IMUData& imu_data) {
  static int count = 0;
  static VelocityData velocity_data;
  static PoseData last_laser_pose = laser_odom;
  static PoseData last_gnss_pose = gnss_pose;
  ...
  if (key_frames_deque_.size() == 0) {
    ...
    if (odo_pre_integrator_ptr_) {
      gnss_pose.GetVelocityData(velocity_data);
      odo_pre_integrator_ptr_->Init(velocity_data);
    }

    last_laser_pose = laser_odom;
    last_gnss_pose = gnss_pose;

    has_new_key_frame_ = true;
  }

  // whether the current scan is far away enough from last key frame:
  if (fabs(laser_odom.pose_(0, 3) - last_laser_pose.pose_(0, 3)) +
          fabs(laser_odom.pose_(1, 3) - last_laser_pose.pose_(1, 3)) +
          fabs(laser_odom.pose_(2, 3) - last_laser_pose.pose_(2, 3)) >
      key_frame_distance_) {
    if (imu_pre_integrator_ptr_) {
    ...
    if (0 == (++count) % 10) {
      ...
      ShowIMUPreIntegrationResidual(
          last_gnss_pose, gnss_pose, imu_pre_integration_);
  ...
  if (has_new_key_frame_) {
    ...
    current_key_gnss_.pose_ = gnss_pose.pose_;
    current_key_gnss_.vel_.v = gnss_pose.vel_.v;
    current_key_gnss_.vel_.w = gnss_pose.vel_.w;
    ...
    pre_integration_deque_.push_back(imu_pre_integration_);
  }

  return has_new_key_frame_;
}

bool LIOBackEndSim::AddNodeAndEdge(const PoseData& gnss_data) {
  ...
  if (N > 1 && graph_optimizer_config_.use_odo_pre_integration) {
    ...
    graph_optimizer_ptr_->AddPRVAGOdoPreIntegrationEdge(
        vertex_index_i, vertex_index_j, odo_pre_integration_);
  }
...
bool LIOBackEndSim::MaybeOptimized() {
...
bool LIOBackEndSim::SavePose(const Eigen::Matrix4f& pose, std::ofstream& ofs) {
  for (int i = 0; i < 3; ++i) {
    for (int j = 0; j < 4; ++j) {
      ofs << pose(i, j);
...
bool LIOBackEndSim::ForceOptimize() {
...
bool LIOBackEndSim::SaveOptimizedPose() {
  ...
  if (!FileManager::CreateFile(trajectory_path_ + "/ground_truth.txt",
      ...
      !FileManager::CreateFile(trajectory_path_ + "/optimized.txt",
                               optimized_pose_ofs_) ||
      !FileManager::CreateFile(trajectory_path_ + "/pure.txt",
                               pure_integration_ofs_)) {
    return false;
  }

  graph_optimizer_ptr_->GetOptimizedKeyFrame(optimized_key_frames_);

  Eigen::Matrix4d inter_pose = key_gnss_deque_.at(0).pose_.cast<double>();
  Eigen::Vector3d inter_vel = key_gnss_deque_.at(0).vel_.v.cast<double>();
  ...
  for (size_t i = 0; i < optimized_key_frames_.size(); ++i) {
    ...
    // save the pure imu integration
    IMUPreIntegrator::IMUPreIntegration integration_temp =
        pre_integration_deque_.at(i);
    Eigen::Matrix3d prev_R = inter_pose.block<3, 3>(0, 0);
    Eigen::Vector3d prev_t = inter_pose.block<3, 1>(0, 3);
    Eigen::Vector3d prev_v = inter_vel;
    Eigen::Vector3d g = integration_temp.g;
    double dT = integration_temp.T;

    inter_pose.block<3, 1>(0, 3) = prev_t + prev_v * dT - 0.5 * g * dT * dT +
                                   prev_R * integration_temp.alpha_ij;
    inter_vel = inter_vel - g * dT + prev_R * integration_temp.beta_ij;
    inter_pose.block<3, 3>(0, 0) = prev_R * integration_temp.theta_ij.matrix();

    SavePose(inter_pose.cast<float>(), pure_integration_ofs_);
  }

  // LOG(INFO) << "gravity: " << pre_integration_deque_.at(1).g.x() << ", "
  //           << pre_integration_deque_.at(1).g.y() << ", "
  //           << pre_integration_deque_.at(1).g.z() << ", " << std::endl
  //           << "init_vel: " << optimized_key_frames_.at(0).vel_.v.x() << ", "
  //           << optimized_key_frames_.at(0).vel_.v.y() << ", "
  //           << optimized_key_frames_.at(0).vel_.v.z() << ", " << std::endl
  //           << "true_init_vel: " << key_gnss_deque_.at(0).vel_.v.x() << ", "
  //           << key_gnss_deque_.at(0).vel_.v.y() << ", "
  //           << key_gnss_deque_.at(0).vel_.v.z() << ", " << std::endl;

  return true;
}

void LIOBackEndSim::GetOptimizedKeyFrames(
    std::deque<KeyFrame>& key_frames_deque) {
  key_frames_deque.clear();

  key_frames_deque.insert(key_frames_deque.end(),
                          optimized_key_frames_.begin(),
                          optimized_key_frames_.end());
}

void LIOBackEndSim::GetLatestKeyScan(CloudData& key_scan) {
  key_scan.time_ = current_key_scan_.time_;
  key_scan.cloud_ptr_.reset(
      new CloudData::CloudType(*current_key_scan_.cloud_ptr_));
}

void LIOBackEndSim::ShowIMUPreIntegrationResidual(
  ...
  LOG(INFO) << "IMU Pre-Integration Measurement: " << std::endl
            ...
            << "\tgravity:" << imu_pre_integration.g.x() << ", "
            << imu_pre_integration.g.y() << ", " << imu_pre_integration.g.z()
            << std::endl
            << "\tcovariance:" << std::endl
            ...
            << imu_pre_integration.P(14, 14)
            << std::endl
            /*
            << "\tJacobian:" << std::endl
            << "\t\td_alpha_d_b_a: " << std::endl
            << "\t\t\t" << imu_pre_integration.J_( 0,  9) << ", " <<
            imu_pre_integration.J_( 0, 10) << ", " << imu_pre_integration.J_( 0,
            11) << std::endl
            << "\t\t\t" << imu_pre_integration.J_( 1,  9) << ", " <<
            imu_pre_integration.J_( 1, 10) << ", " << imu_pre_integration.J_( 1,
            11) << std::endl
            << "\t\t\t" << imu_pre_integration.J_( 2,  9) << ", " <<
            imu_pre_integration.J_( 2, 10) << ", " << imu_pre_integration.J_( 2,
            11) << std::endl
            << "\t\td_alpha_d_b_g: " << std::endl
            << "\t\t\t" << imu_pre_integration.J_( 0, 12) << ", " <<
            imu_pre_integration.J_( 0, 13) << ", " << imu_pre_integration.J_( 0,
            14) << std::endl
            << "\t\t\t" << imu_pre_integration.J_( 1, 12) << ", " <<
            imu_pre_integration.J_( 1, 13) << ", " << imu_pre_integration.J_( 1,
            14) << std::endl
            << "\t\t\t" << imu_pre_integration.J_( 2, 12) << ", " <<
            imu_pre_integration.J_( 2, 13) << ", " << imu_pre_integration.J_( 2,
            14) << std::endl
            << "\t\td_theta_d_b_g: " << std::endl
            << "\t\t\t" << imu_pre_integration.J_( 3, 12) << ", " <<
            imu_pre_integration.J_( 3, 13) << ", " << imu_pre_integration.J_( 3,
            14) << std::endl
            << "\t\t\t" << imu_pre_integration.J_( 4, 12) << ", " <<
            imu_pre_integration.J_( 4, 13) << ", " << imu_pre_integration.J_( 4,
            14) << std::endl
            << "\t\t\t" << imu_pre_integration.J_( 5, 12) << ", " <<
            imu_pre_integration.J_( 5, 13) << ", " << imu_pre_integration.J_( 5,
            14) << std::endl
            << "\t\td_beta_d_b_a: " << std::endl
            << "\t\t\t" << imu_pre_integration.J_( 6,  9) << ", " <<
            imu_pre_integration.J_( 6, 10) << ", " << imu_pre_integration.J_( 6,
            11) << std::endl
            << "\t\t\t" << imu_pre_integration.J_( 7,  9) << ", " <<
            imu_pre_integration.J_( 7, 10) << ", " << imu_pre_integration.J_( 7,
            11) << std::endl
            << "\t\t\t" << imu_pre_integration.J_( 8,  9) << ", " <<
            imu_pre_integration.J_( 8, 10) << ", " << imu_pre_integration.J_( 8,
            11) << std::endl
            << "\t\td_beta_d_b_a: " << std::endl
            << "\t\t\t" << imu_pre_integration.J_( 6, 12) << ", " <<
            imu_pre_integration.J_( 6, 13) << ", " << imu_pre_integration.J_( 6,
            14) << std::endl
            << "\t\t\t" << imu_pre_integration.J_( 7, 12) << ", " <<
            imu_pre_integration.J_( 7, 13) << ", " << imu_pre_integration.J_( 7,
            14) << std::endl
            << "\t\t\t" << imu_pre_integration.J_( 8, 12) << ", " <<
            imu_pre_integration.J_( 8, 13) << ", " << imu_pre_integration.J_( 8,
            14) << std::endl
            */
            << std::endl;
}
