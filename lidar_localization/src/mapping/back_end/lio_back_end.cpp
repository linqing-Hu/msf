 ...
 * @LastEditTime: 2022-11-02 22:03:27
 */
#include "lidar_localization/mapping/back_end/lio_back_end.h"

namespace lidar_localization {

LIOBackEnd::LIOBackEnd(const std::string& config_file_path) {
  InitWithConfig(config_file_path);
}

bool LIOBackEnd::InitWithConfig(const std::string& config_file_path) {
...
bool LIOBackEnd::Update(const CloudData& cloud_data,
...
bool LIOBackEnd::MaybeNewKeyFrame(const CloudData& cloud_data,
                                  const PoseData& laser_odom,
  ...
  if ((laser_odom.pose_.block<3, 1>(0, 3) -
    ...
    if (0 == (++count) % 10) {
      ...
      ShowIMUPreIntegrationResidual(
          last_gnss_pose, gnss_odom, imu_pre_integration_);
...
bool LIOBackEnd::SavePose(const Eigen::Matrix4f& pose, std::ofstream& ofs) {
  for (int i = 0; i < 3; ++i) {
    for (int j = 0; j < 4; ++j) {
      ofs << pose(i, j);
