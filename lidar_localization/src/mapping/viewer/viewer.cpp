 ...
 * @LastEditTime: 2022-09-25 22:30:47
 */
...
bool Viewer::UpdateWithNewKeyFrame(const PoseData& transformed_data,
  ...
  pcl::transformPointCloud(*cloud_data.cloud_ptr_,
...
bool Viewer::JointGlobalMap(CloudData::CloudTypePtr& global_map_ptr) {
  JointCloudMap(optimized_key_frames_, global_map_ptr);
  return true;
}

bool Viewer::JointLocalMap(CloudData::CloudTypePtr& local_map_ptr) {
...
const Eigen::Matrix4f& Viewer::GetCurrentPose() const {
  return optimized_odom_.pose_;
}

const CloudData::CloudTypePtr& Viewer::GetCurrentScan() {
  frame_filter_ptr_->Filter(optimized_cloud_.cloud_ptr_,
                            optimized_cloud_.cloud_ptr_);
  return optimized_cloud_.cloud_ptr_;
}

bool Viewer::GetLocalMap(CloudData::CloudTypePtr& local_map_ptr) {
  JointLocalMap(local_map_ptr);
  local_map_filter_ptr_->Filter(local_map_ptr, local_map_ptr);
  return true;
}

bool Viewer::GetGlobalMap(CloudData::CloudTypePtr& global_map_ptr) {
