...
int main(int argc, char* argv[]) {
  ...
  std::shared_ptr<ShenLanLIOBackEndFlow> shenlan_lio_back_end_flow_ptr =
      std::make_shared<ShenLanLIOBackEndFlow>(nh, cloud_topic, odom_topic);
  ...
  while (ros::ok()) {
    ...
    shenlan_lio_back_end_flow_ptr->Run();

    if (need_optimize_map_) {
      shenlan_lio_back_end_flow_ptr->ForceOptimize();
      shenlan_lio_back_end_flow_ptr->SaveOptimizedOdometry();
