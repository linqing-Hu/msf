 ...
 * @LastEditTime: 2022-09-22 10:00:52
 */

#include "lidar_localization/publisher/lidar_measurement_publisher.h"
...
LidarMeasurementPublisher::LidarMeasurementPublisher(
  ...
  lidar_measurement_.gnss_odometry.header.frame_id =
      gnss_odometry_pos_frame_id_;
  lidar_measurement_.gnss_odometry.child_frame_id = gnss_odometry_vel_frame_id_;

  publisher_ = nh_.advertise<lidar_localization::LidarMeasurement>(topic_name,
                                                                   buff_size);
}
...
void LidarMeasurementPublisher::Publish(
  ...
  PublishData(
      ros_time, cloud_data_ptr, imu_data, transform_matrix, velocity_data);
}
...
void LidarMeasurementPublisher::Publish(
  ...
  PublishData(time, cloud_data_ptr, imu_data, transform_matrix, velocity_data);
}
...
void LidarMeasurementPublisher::SetIMU(const ros::Time& time,
                                       const IMUData& imu_data,
  ...
  // set orientation:
  imu.orientation.w = imu_data.orientation_.w;
  imu.orientation.x = imu_data.orientation_.x;
  imu.orientation.y = imu_data.orientation_.y;
  imu.orientation.z = imu_data.orientation_.z;

  // set angular velocity:
  imu.angular_velocity.x = imu_data.angular_velocity_.x;
  imu.angular_velocity.y = imu_data.angular_velocity_.y;
  imu.angular_velocity.z = imu_data.angular_velocity_.z;

  // set linear acceleration:
  imu.linear_acceleration.x = imu_data.linear_acceleration_.x;
  imu.linear_acceleration.y = imu_data.linear_acceleration_.y;
  imu.linear_acceleration.z = imu_data.linear_acceleration_.z;
}
...
void LidarMeasurementPublisher::SetGNSSOdometry(
  ...
  // set the twist:
  gnss_odometry.twist.twist.linear.x = velocity_data.linear_velocity_.x;
  gnss_odometry.twist.twist.linear.y = velocity_data.linear_velocity_.y;
  gnss_odometry.twist.twist.linear.z = velocity_data.linear_velocity_.z;

  gnss_odometry.twist.twist.angular.x = velocity_data.angular_velocity_.x;
  gnss_odometry.twist.twist.angular.y = velocity_data.angular_velocity_.y;
  gnss_odometry.twist.twist.angular.z = velocity_data.angular_velocity_.z;
}
...
void LidarMeasurementPublisher::PublishData(
  ...
  // c. set GNSS odometry:
  SetGNSSOdometry(
      time, transform_matrix, velocity_data, lidar_measurement_.gnss_odometry);
