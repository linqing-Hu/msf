 ...
 * @LastEditTime: 2022-10-03 22:17:21
 */
...
void Activity::Init() {
  // parse IMU config:
  private_nh_.param("imu/device_name",
                    imu_config_.device_name,
                    std::string("GNSS_INS_SIM_IMU"));
  private_nh_.param(
      "imu/topic_name", imu_config_.topic_name, std::string("/sim/sensor/imu"));
  ...
  private_nh_.param(
      "imu/bias/angular_velocity/x", imu_config_.bias.angular_velocity.x, 0.0);
  private_nh_.param(
      "imu/bias/angular_velocity/y", imu_config_.bias.angular_velocity.y, 0.0);
  private_nh_.param(
      "imu/bias/angular_velocity/z", imu_config_.bias.angular_velocity.z, 0.0);
  ...
  private_nh_.param("imu/bias/linear_acceleration/x",
                    imu_config_.bias.linear_acceleration.x,
                    0.0);
  private_nh_.param("imu/bias/linear_acceleration/y",
                    imu_config_.bias.linear_acceleration.y,
                    0.0);
  private_nh_.param("imu/bias/linear_acceleration/z",
                    imu_config_.bias.linear_acceleration.z,
                    0.0);
  ...
  private_nh_.param(
      "imu/gyro/sigma_noise", imu_config_.gyro_noise_stddev, 0.015);
  ...
  // parse odom config:
  private_nh_.param(
      "pose/frame_id", odom_config_.frame_id, std::string("inertial"));
  private_nh_.param("pose/topic_name",
                    odom_config_.topic_name.ground_truth,
                    std::string("/pose/ground_truth"));

  LOG(INFO) << "Simulator: " << std::endl
            << " imu topic: " << imu_config_.topic_name << std::endl
            << " ground truth topic: " << odom_config_.topic_name.ground_truth
            << std::endl
            << " imu gravity: " << imu_config_.gravity.x << " "
            << imu_config_.gravity.y << " " << imu_config_.gravity.z
            << std::endl;

  // init publishers:
  pub_imu_ =
      private_nh_.advertise<sensor_msgs::Imu>(imu_config_.topic_name, 500);
  pub_odom_ = private_nh_.advertise<nav_msgs::Odometry>(
      odom_config_.topic_name.ground_truth, 500);
...
void Activity::GetGroundTruth() {
  // acceleration:
  const double timestamp_in_sec = timestamp_.toSec();
  const double sin_w_xy_t = sin(kOmegaXY * timestamp_in_sec);
  const double cos_w_xy_t = cos(kOmegaXY * timestamp_in_sec);
  const double sin_w_z_t = sin(kOmegaZ * timestamp_in_sec);
  const double cos_w_z_t = cos(kOmegaZ * timestamp_in_sec);
  const double rho_x_w_xy = kRhoX * kOmegaXY;
  const double rho_y_w_xy = kRhoY * kOmegaXY;
  const double rho_z_w_z = kRhoZ * kOmegaZ;

  const Eigen::Vector3d p(
      kRhoX * cos_w_xy_t, kRhoY * sin_w_xy_t, kRhoZ * sin_w_z_t);
  const Eigen::Vector3d v(
      -rho_x_w_xy * sin_w_xy_t, rho_y_w_xy * cos_w_xy_t, rho_z_w_z * cos_w_z_t);
  const Eigen::Vector3d a(-rho_x_w_xy * kOmegaXY * cos_w_xy_t,
                          -rho_y_w_xy * kOmegaXY * sin_w_xy_t,
                          -rho_z_w_z * kOmegaZ * sin_w_z_t);

  // angular velocity:
  const double sin_t = sin(timestamp_in_sec);
  const double cos_t = cos(timestamp_in_sec);

  Eigen::Vector3d euler_angles(
      kRoll * cos_t, kPitch * sin_t, kYaw * timestamp_in_sec);

  Eigen::Vector3d euler_angle_rates(-kRoll * sin_t, kPitch * cos_t, kYaw);
  ...
  // a. angular velocity:
  angular_vel_ =
      EulerAngleRatesToBodyAngleRates(euler_angles, euler_angle_rates);
...
void Activity::AddNoise(double delta_t) {
  ...
  const double sqrt_delta_t = sqrt(delta_t);

  // a. update bias: 加连续时间的噪声
  angular_vel_bias_ +=
      GetGaussianNoise(imu_config_.gyro_bias_stddev * sqrt_delta_t);
  linear_acc_bias_ +=
      GetGaussianNoise(imu_config_.acc_bias_stddev * sqrt_delta_t);

  // b. get measurement noise:
  const Eigen::Vector3d angular_vel_noise =
      GetGaussianNoise(imu_config_.gyro_noise_stddev / sqrt_delta_t);
  const Eigen::Vector3d linear_acc_noise =
      GetGaussianNoise(imu_config_.acc_noise_stddev / sqrt_delta_t);
...
void Activity::SetOdometryMessage() {
  ...
  message_odom_.header.frame_id = "world";
...
Eigen::Matrix3d Activity::EulerAnglesToRotation(
  ...
  // parse Euler angles:
  const double roll = euler_angles.x();
  ...
  R_ib << cy * cp, cy * sp * sr - sy * cr, sy * sr + cy * cr * sp, sy * cp,
...
Eigen::Vector3d Activity::EulerAngleRatesToBodyAngleRates(
  ...
  // parse euler angles:
  const double roll = euler_angles(0);
  const double pitch = euler_angles(1);
