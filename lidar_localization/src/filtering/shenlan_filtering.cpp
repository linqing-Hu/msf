 ...
 * @LastEditTime: 2022-11-05 23:34:31
 */
#include "lidar_localization/filtering/shenlan_filtering.h"

namespace lidar_localization {

ShenLanFiltering::ShenLanFiltering()
...
bool ShenLanFiltering::InitWithConfig(const CloudData& init_scan,
                                      const Eigen::Vector3f& init_vel,
                                      const IMUData& init_imu_data) {
  ResetLocalMap(relocalization_guess_pos_.x(),
...
bool ShenLanFiltering::Relocalization(
  ...
  for (size_t i = 0; i < N; i++) {
    ...
    relocalization_registration_ptr_->ScanMatch(
        current_scan_ptr, guess_pose, result_cloud_ptr, result_pose);
...
bool ShenLanFiltering::InitWithGNSS(const Eigen::Matrix4f& init_pose,
                                    const Eigen::Vector3f& init_vel,
                                    const IMUData& init_imu_data) {
...
bool ShenLanFiltering::Init(const CloudData& init_scan,
...
bool ShenLanFiltering::Update(const IMUData& imu_data) {
...
bool ShenLanFiltering::Correct(const IMUData& imu_data,
  ...
  registration_ptr_->ScanMatch(
      filtered_cloud_ptr, current_pose_, result_cloud_ptr, cloud_pose);
...
void ShenLanFiltering::GetGlobalMap(CloudData::CloudTypePtr& global_map) {
  // downsample global map for visualization:
  global_map_filter_ptr_->Filter(global_map_ptr_, global_map);
  has_new_global_map_ = false;
}

void ShenLanFiltering::GetOdometry(Eigen::Matrix4f& pose,
                                   Eigen::Vector3f& vel) {
...
bool ShenLanFiltering::InitWithConfig() {
...
bool ShenLanFiltering::InitFilter(
...
bool ShenLanFiltering::InitLocalMapSegmenter(const YAML::Node& config_node) {
  local_map_segmenter_ptr_ = std::make_shared<BoxFilter>(config_node);
  return true;
}

bool ShenLanFiltering::InitFilters(const YAML::Node& config_node) {
...
bool ShenLanFiltering::InitGlobalMap(const YAML::Node& config_node) {
...
bool ShenLanFiltering::InitRegistration(
    const YAML::Node& config_node,
  ...
  if (registration_method == "NDT") {
    ...
    registration_ptr =
        std::make_shared<Point2PlaneICP>(config_node[registration_method]);
  } else {
...
bool ShenLanFiltering::InitFusion(const YAML::Node& config_node) {
...
bool ShenLanFiltering::InitRelocalization(const YAML::Node& config_node) {
  relocalization_registration_ptr_ =
      std::make_shared<NDTRegistration>(config_node["relocalization"]);
...
bool ShenLanFiltering::SetInitGNSS(const Eigen::Matrix4f& gnss_pose) {
...
bool ShenLanFiltering::SetInitPose(const Eigen::Matrix4f& init_pose) {
...
bool ShenLanFiltering::ResetLocalMap(const float x,
                                     const float y,
                                     const float z) {
